...
func TestPiPaneLooksLikeAClaudePane(t *testing.T) {
	...
	h.waitFor(func() bool { return h.countRows("· ▌ ai: deploy - kido") == 2 }, settle,
		func() string {
	...
	h.waitFor(func() bool {
		return h.countRows("· ▌ ai: deploy - kido") == 1 && h.countRows("· node") == 1
	}, settle, func() string {
...
func TestPiReportedTitleWinsOverPaneTitle(t *testing.T) {
	...
	h.waitGlyph("deploy", "")
	if got := h.rowFor("deploy"); got != "·   ai: deploy" {
...
func TestPiBeatsClaudeOnTheSamePane(t *testing.T) {
	...
	if got := h.rowFor("bridge - kido"); got != "· ▌ ai: bridge - kido" {
